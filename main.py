...
from telegram.helpers import escape_markdown # [cite: 11]
...
# Проверка наличия всех необходимых переменных окружения [cite: 26, 27]
...
# --- Инициализация Gemini API --- [cite: 27]
    ...
    if conn:
        try:
            ...
            if cursor.fetchone() is None:
                ...
                # Если пользователь существует, можно обновить данные, если нужно
                # Или просто пропустить, если не нужно обновлять
        ...
        try:
            ...
            cursor.execute(
                ...
                (user_id, username, first_name, last_name, country, city, phone_number,
                 genres, years, keywords, gemini_response) # [cite: 33]
            )
            conn.commit()
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await save_user_data(user.id, user.username, user.first_name, user.last_name) # Сохраняем данные пользователя
...
async def select_genres(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query.data.startswith("genre_"):
        ...
        keyboard.append([InlineKeyboardButton("⬅️ Назад к жанрам", callback_data="back_to_genres")]) # Кнопка "Назад"
...
async def select_years(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query.data.startswith("year_"):
        ...
        keyboard = [[InlineKeyboardButton("⬅️ Назад к годам", callback_data="back_to_years")]] # Кнопка "Назад"
        reply_markup = InlineKeyboardMarkup(keyboard)
...
async def handle_keywords(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text("Надеюсь, вам что-то подойдет!", reply_markup=reply_markup) # [cite: 16]
...
def main() -> None:
    ...
    conv_handler = ConversationHandler(
        entry_points=[CommandHandler("start", start)], # [cite: 44]
        states={
            SELECT_GENRES: [
                CallbackQueryHandler(select_genres, pattern=r"^(genre_|confirm_genres)$"),
                CallbackQueryHandler(back_to_genres, pattern="^back_to_genres$") # Добавляем обработчик для кнопки "Назад"
            ],
            SELECT_YEARS: [
                CallbackQueryHandler(select_years, pattern=r"^(year_|confirm_years)$"),
                CallbackQueryHandler(back_to_genres, pattern="^back_to_genres$") # "Назад" из годов в жанры
            ],
            ENTER_KEYWORDS: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, handle_keywords),
                CallbackQueryHandler(back_to_years, pattern="^back_to_years$") # "Назад" из ключевых слов в года
            ],
        },
        fallbacks=[CommandHandler("cancel", cancel), MessageHandler(filters.COMMAND | filters.TEXT, unknown)], # [cite: 45]
    )

    application.add_handler(conv_handler)
    application.add_handler(CallbackQueryHandler(start, pattern="^start_over$")) # [cite: 24] для кнопки "Начать новый поиск"
    application.add_handler(MessageHandler(filters.COMMAND, unknown)) # [cite: 45]

    logger.info("Бот запущен. Ожидание сообщений...") # [cite: 45]
    application.run_polling(allowed_updates=Update.ALL_TYPES) # [cite: 45]
